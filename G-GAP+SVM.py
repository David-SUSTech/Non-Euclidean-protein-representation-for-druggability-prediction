...
class ProteinClassifier:
        ...
        for kernel in param_grid['kernel']:
            if kernel == 'linear':
                for C in param_grid['C']:
                    param_combinations.append({'kernel': kernel, 'C': C})
            elif kernel == 'rbf':
                for C in param_grid['C']:
                    for gamma in param_grid['gamma']:
                        param_combinations.append({'kernel': kernel, 'C': C, 'gamma': gamma})
    ...
    def train_and_evaluate(self, X, y):
        ...
        self.optimal_params = self.find_optimal_svm_params_loocv(X_scaled, y, param_grid_for_search)
        ...
        # 为最终评估模型启用概率预测
        final_params = self.optimal_params.copy()
        final_params['probability'] = True

        final_classifier = SVC(**final_params, random_state=self.random_state)
